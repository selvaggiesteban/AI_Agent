# ESTEBAN

## References

- [UI Skills](https://www.ui-skills.com/skills/)
- [Glama MCP Servers](https://glama.ai/mcp/servers?attributes=author%3Aofficial&sort=github-stargazers%3Adesc)
- [Skills.sh](https://www.skills.sh/)

## Utility Repositories

- [gpt-researcher](https://github.com/assafelovic/gpt-researcher)
- [skills (Anthropic)](https://github.com/anthropics/skills)
- [caveman](https://github.com/JuliusBrussee/caveman)
- [last30days-skill](https://github.com/mvanhorn/last30days-skill)
- [free-claude-code](https://github.com/Alishahryar1/free-claude-code)
- [claude-ads](https://github.com/AgriciDaniel/claude-ads)
- [humanizer](https://github.com/blader/humanizer)
- [social-media-skills](https://github.com/charlie947/social-media-skills)
- [claude-seo](https://github.com/AgriciDaniel/claude-seo)
- [superpowers](https://github.com/obra/superpowers)
- [agentic-inbox (Cloudflare)](https://github.com/cloudflare/agentic-inbox)
- [nango](https://github.com/NangoHQ/nango)
- [supabase](https://github.com/supabase/supabase)
- [MoneyPrinterTurbo](https://github.com/harry0703/MoneyPrinterTurbo)
- [codegraph](https://github.com/colbymchenry/codegraph)
- [Anthropic-Cybersecurity-Skills](https://github.com/mukul975/Anthropic-Cybersecurity-Skills)
- [Scrapling](https://github.com/D4Vinci/Scrapling)
- [obsidian-mind](https://github.com/breferrari/obsidian-mind)
- [poisson_for_products](https://github.com/selvaggiesteban/poisson_for_products)
- [30_days_coding](https://github.com/selvaggiesteban/30_days_coding)
- [spec-driven_development](https://github.com/selvaggiesteban/spec-driven_development)
- [hands-on-ai](https://github.com/selvaggiesteban/hands-on-ai)
- [ponytail](https://github.com/DietrichGebert/ponytail)
- [markitdown (Microsoft)](https://github.com/microsoft/markitdown)
- [codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)
- [schemaorg](https://github.com/schemaorg/schemaorg)

## MCP Servers

### Documentation & Guides

- [Vercel MCP](https://vercel.com/docs/agent-resources/vercel-mcp)
- [Playwright MCP (Microsoft)](https://github.com/microsoft/playwright-mcp)
- [Figma MCP Server](https://help.figma.com/hc/en-us/articles/32132100833559-Guide-to-the-Figma-MCP-server)
- [Firecrawl MCP Server](https://docs.firecrawl.dev/mcp-server)
- [GitHub MCP Server](https://github.com/github/github-mcp-server)
- [Supabase MCP](https://supabase.com/docs/guides/ai-tools/mcp)
- [Stripe MCP](https://docs.stripe.com/mcp?mcp-client=claudecode)

### Official MCP Servers

- [Fetch](https://github.com/modelcontextprotocol/servers/blob/main/src/fetch/README.md)
- [SQLite](https://github.com/modelcontextprotocol/servers/tree/main/src/sqlite)
- [Postgres](https://github.com/modelcontextprotocol/servers/tree/main/src/postgres)
- [GitHub MCP](https://github.com/github/github-mcp-server) — PRs, issues, commits, code review
- [Playwright MCP](https://github.com/microsoft/playwright-mcp) — Browser automation and testing
- [Sentry MCP](https://github.com/getsentry/sentry-mcp) — Error monitoring
- [Cloudflare MCP](https://github.com/cloudflare/mcp-server-cloudflare) — Workers, KV, D1, R2
- [Supabase MCP](https://supabase.com/docs/guides/ai-tools/mcp) — Database and auth

### Third-Party MCP Servers

- [Chrome DevTools MCP](https://github.com/chromedevtools/chrome-devtools-mcp)
- [gpt-researcher MCP](https://github.com/assafelovic/gpt-researcher)
- [FastAPI MCP](https://github.com/tadata-org/fastapi_mcp)
- [AWS Labs MCP](https://github.com/awslabs/mcp)
- [NotebookLM Skill](https://github.com/pleaseprompto/notebooklm-skill)
- [WhatsApp MCP](https://github.com/lharries/whatsapp-mcp)
- [MCP Atlassian](https://github.com/sooperset/mcp-atlassian)
- [Google Ads MCP](https://github.com/googleads/google-ads-mcp)
- [Codegraph MCP](https://github.com/colbymchenry/codegraph)
- [Context7 (Upstash)](https://github.com/upstash/context7)
- [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR)
- [Meta MCP CLI](https://claudefa.st/blog/tools/mcp-extensions/meta-mcp-cli)
- [Cloudflare MCP Server](https://github.com/cloudflare/mcp-server-cloudflare)
- [Apple MCP (SuperMemory)](https://github.com/supermemoryai/apple-mcp)
- [LinkedIn MCP Server](https://github.com/stickerdaniel/linkedin-mcp-server)
- [Google Analytics MCP](https://github.com/googleanalytics/google-analytics-mcp)
- [MCP on Vercel](https://github.com/vercel-labs/mcp-on-vercel)
- [Playwright MCP (Cloudflare)](https://github.com/cloudflare/playwright-mcp)
- [Postman MCP Server](https://github.com/postmanlabs/postman-mcp-server)
- [DebugMCP (Microsoft)](https://github.com/microsoft/DebugMCP)
- [Redis MCP](https://github.com/redis/mcp-redis)
- [Cloudflare MCP](https://github.com/cloudflare/mcp)
- [Cloud Run MCP](https://github.com/googlecloudplatform/cloud-run-mcp)
- [Workers MCP (Cloudflare)](https://github.com/cloudflare/workers-mcp)
- [Sentry MCP](https://github.com/getsentry/sentry-mcp)
- [Next DevTools MCP (Vercel)](https://github.com/vercel/next-devtools-mcp)
- [X MCP (Twitter)](https://github.com/xdevplatform/xmcp)
- [WordPress MCP](https://github.com/Automattic/wordpress-mcp)
- [Local SEO Data MCP](https://github.com/localseodata/mcp-server)
- [Falcon MCP (CrowdStrike)](https://github.com/crowdstrike/falcon-mcp)
- [PayPal Agent Toolkit MCP](https://github.com/paypal/agent-toolkit/tree/main/modelcontextprotocol)
- [Docker Hub MCP](https://github.com/docker/hub-mcp)
- [Hostinger API MCP](https://github.com/hostinger/api-mcp-server)
- [Astro Docs MCP](https://github.com/withastro/docs-mcp)
- [Groq Compound MCP](https://github.com/groq/compound-mcp-server)
- [Cerebras Code MCP](https://github.com/Cerebras/cerebras-code-mcp)
- [MailboxValidator MCP](https://github.com/MailboxValidator/mcp-mailboxvalidator)

## Skills

- `skills/ads_strategist.md` — TOFU/MOFU/BOFU copys
- `skills/google-ads-audit.md` — Campaigns analysis

## Scripts

- `scripts/#/` — Normalized database
- `scripts/seo_manager/seo-content-generator/seo_content_generator.py` — SEO content
- `scripts/database_manager/audit_db.py` — Contacts.db audits
- `scripts/database_manager/imap_bot.py` — Analyze email inboxes
- `scripts/e-mail_marketing_manager/e-mail.py` — E-mail marketing engine
- `scripts/e-mail_marketing_manager/form-tester/main.py` — Test web forms
- `scripts/e-mail_marketing_manager/domain_status_checker/domain_status_checker.py` — Check domain health and activity
- `scripts/web_scraper/smart-research-assistant/main.py` — SEO blog articles
- `scripts/web_designer/landing_page.py` — Create landing pages
- `scripts/account_executive/trello.py` — Analyze Trello boards
- `scripts/check_providers.py` — Check LLM availability

## Plugins

### Claude Code

- [mcp-server-dev](https://github.com/anthropics/claude-plugins-official) — MCP server scaffolding
- [Superpowers](https://github.com/obra/superpowers) — Skill suite for AI engineering
- [Skill Creator](https://claude.com/plugins) — Create and distribute custom skills
- [Claude Code Setup](https://claude.com/plugins) — Initial setup and best practices
- [Plugin Developer Toolkit](https://claude.com/plugins) — Plugin development
- [Frontend Design](https://claude.com/plugins) — Frontend design with Figma integration
- [postman](https://claude.com/plugins) — API testing with Postman
- [PostHog](https://claude.com/plugins) — Analytics and event tracking
- [Qodo Skills](https://claude.com/plugins) — Code review and testing
- [Datadog](https://claude.com/plugins) — Monitoring and observability

### OpenCode

- [opencode-daytona](https://github.com/daytonaio/daytona/tree/main/libs/opencode-plugin) — Isolated sandboxes with git sync and live previews
- [opencode-firecrawl](https://github.com/firecrawl/opencode-firecrawl) — Web scraping, crawling, and search via Firecrawl CLI
- [opencode-sentry-monitor](https://github.com/stolinski/opencode-sentry-monitor) — Agent trace and debug with Sentry AI Monitoring
- [opencode-jfrog-plugin](https://github.com/jfrog/opencode-jfrog-plugin) — Integration with JFrog platform
- [opencode-openai-codex-auth](https://github.com/numman-ali/opencode-openai-codex-auth) — Use ChatGPT Plus/Pro subscription
- [opencode-gemini-auth](https://github.com/jenslys/opencode-gemini-auth) — Use existing Gemini plan
- [opencode-antigravity-auth](https://github.com/NoeFabris/opencode-antigravity-auth) — Free models via Antigravity
- [opencode-dynamic-context-pruning](https://github.com/Tarquinen/opencode-dynamic-context-pruning) — Token optimization by pruning obsolete outputs
- [opencode-vibeguard](https://github.com/inkdust2021/opencode-vibeguard) — Redaction of secrets/PII before LLM calls
- [opencode-pty](https://github.com/shekohex/opencode-pty) — Background processes in PTY with interactive input
- [opencode-supermemory](https://github.com/supermemoryai/opencode-supermemory) — Persistent memory between sessions
- [opencode-background-agents](https://github.com/kdcokenny/opencode-background-agents) — Claude Code-style background agents
- [opencode-workspace](https://github.com/kdcokenny/opencode-workspace) — Multi-agent orchestration (16 components)
- [opencode-worktree](https://github.com/kdcokenny/opencode-worktree) — Frictionless git worktrees
- [opencode-scheduler](https://github.com/different-ai/opencode-scheduler) — Recurring jobs with cron syntax
- [opencode-conductor](https://github.com/derekbar90/opencode-conductor) — Workflow Context $\rightarrow$ Spec $\rightarrow$ Plan $\rightarrow$ Implement
- [opencode-wakatime](https://github.com/angristan/opencode-wakatime) — Usage tracking with Wakatime
- [opencode-notificator](https://github.com/panta82/opencode-notificator) — Desktop notifications and sound alerts
