...
def get_session() -> requests.Session:
    """Configures and returns a requests session with random headers"""
...
def extract_page_data(url: str, session: requests.Session) -> List[Dict[str, str]]:
    """Extracts company data from a specific Kompass URL"""
...
def extract_kompass_data(sector: str, sector_id: str, num_pages: int) -> List[Dict[str, str]]:
    """Coordinates multi-threaded scraping of multiple Kompass pages"""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for page in range(1, num_pages + 1):
            ...
            time.sleep(random.uniform(2, 5))  # Pause between requests
...
def save_to_csv(data: List[Dict[str, str]], filename: str) -> None:
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as csv_file:
        fields = ['Name', 'Location', 'Summary', 'Products', 'phone', 'Website']
...
if __name__ == "__main__":
    ...
    save_to_csv(results, 'companies_construction_kompass.csv')
    print(f"Extracted {len(results)} companies and saved to 'companies_construction_kompass.csv'")
