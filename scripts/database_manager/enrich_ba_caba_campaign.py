"""
enrich_ba_caba_campaign.py — Enriquece contacts.db con datos de campana BA/CABA.
...
def parse_ok_emails(logs_dir):
    """Parsea logs BA/CABA y retorna lista de dicts con email, sender, timestamp."""
    results = []
    pattern = os.path.join(logs_dir, "log_ba_ciclo_*.txt")
    import glob
...
def enrich(db_path, entries):
    ...
    cur.execute("SELECT COUNT(*) FROM campaign WHERE list_val LIKE 'BA-CABA%'")
...
if __name__ == "__main__":
    ...
    print("=== Enriquecimiento Campaign BA/CABA-03082026 ===\n")
