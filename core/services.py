...
#
"""
Services Pipeline — Customer lifecycle orchestrator.
"""
...
class ServicesPipeline(BasePipeline):
    ...
    def phase_contrato(self, cliente: str, dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 1: Contract — %s ===", cliente)
        data = self._get_cliente_data(cliente) or {"title": cliente, "primary_email": "", "phone": "", "address": "", "sector": ""}
        template_path = ROOT_DIR / "scripts" / "templates" / "service_agreement.md"
        ...
        template = template.replace("CLIENT: ..........................................................", f"CLIENT: {nombre}")
        template = template.replace("National ID ...................................", "National ID [COMPLETE]")
        template = template.replace("Tax ID ...................................", "Tax ID [COMPLETE]")
        template = template.replace("with address at .........................................................., hereinafter", f"with address at {domicilio or '[COMPLETE]'}, hereinafter")
        today = datetime.now()
        template = template.replace("on the ....... day of the month of ........................... 20....", f"on the {today.day} day of the month of {today.strftime('%B')} of {today.year}")
        if sector: template = template.replace("Project description:\n> .................................................................................", f"Project description:\n> {sector.lower()} services for {nombre}")
        contact_info = f"\n\n---\n> **CLIENT contact details (extracted from DB):**\n"
        if email: contact_info += f"> - Email: {email}\n"
        if telefono: contact_info += f"> - Phone: {telefono}\n"
        if ciudad: contact_info += f"> - City: {ciudad}\n"
        if provincia: contact_info += f"> - Province: {provincia}\n"
        contact_info += f"> - Sector: {sector or 'Not specified'}\n"
        contact_info += "> - **NOTE:** National ID and Tax ID must be completed by the client before signing.\n"
        template += contact_info
        out_dir = DATA_OUTPUTS_DIR / "contracts"
        out_dir.mkdir(parents=True, exist_ok=True)
        slug = cliente.lower().replace(" ", "_").replace(".", "")
        out_path = out_dir / f"{slug}_contract.md"
    ...
    def _setup_landing(self, cliente: str, data: Dict, dry_run: bool) -> Dict:
        self.logger.info("  → Setup Landing Page for %s", cliente)
        ...
        try:
            ...
            self.logger.info("  Landing page generated at: %s", out_dir)
    ...
    def _setup_astro(self, cliente: str, data: Dict, dry_run: bool) -> Dict:
        self.logger.info("  → Setup Astro Site for %s", cliente)
    ...
    def _setup_medusa(self, cliente: str, data: Dict, dry_run: bool) -> Dict:
        self.logger.info("  → Setup MedusaJS for %s", cliente)
        slug = cliente.lower().replace(" ", "_")
        ...
        try:
            ...
            self.logger.info("  MedusaJS scaffoldered at: %s", out_dir)
    ...
    def phase_setup_web(self, cliente: str, web_type: str = "landing", dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 2: Web Setup (%s) — %s ===", web_type, cliente)
    ...
    def phase_seo(self, cliente: str, mode: str = "content", dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 3: SEO (%s) — %s ===", mode, cliente)
        ...
        if mode == "analyze" and site_url:
            try:
                result = subprocess.run([sys.executable, str(seo_gen), "--urls", site_url, "--output", str(out_dir / "report.md"), "--excel", str(out_dir / "report.xlsx")], capture_output=True, text=True, timeout=600, cwd=str(ROOT_DIR))
    ...
    def phase_ads(self, cliente: str, platform: str = "google", dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 4: Ads (%s) — %s ===", platform, cliente)
        data = self._get_cliente_data(cliente) or {"title": cliente, "urls": ""}
    ...
    def phase_contenido(self, cliente: str, dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 5: Audiovisual Content — %s ===", cliente)
        data = self._get_cliente_data(cliente) or {"title": cliente}
        out_dir = DATA_OUTPUTS_DIR / "content" / cliente.lower().replace(" ", "_")
        out_dir.mkdir(parents=True, exist_ok=True)
        if dry_run: return {"ok": True, "dry_run": True}
        brief = {"cliente": data.get("title", cliente), "sector": data.get("sector", ""), "colores": "to be defined", "estilo": "professional"}
    ...
    def phase_entregar(self, cliente: str, dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 6: Delivery — %s ===", cliente)
        data = self._get_cliente_data(cliente) or {"title": cliente, "urls": ""}
        slug = cliente.lower().replace(" ", "_")
        ...
        if site_url and screenshot_script.exists():
            ...
            except Exception as e: self.logger.warning("  Error taking screenshots: %s", e)
        contrato_src = DATA_OUTPUTS_DIR / "contracts" / f"{slug}_contract.md"
        if contrato_src.exists(): shutil.copy2(contrato_src, delivery_dir / "contract.md")
        seo_src = DATA_OUTPUTS_DIR / "seo" / slug
        if seo_src.exists(): shutil.copytree(seo_src, delivery_dir / "seo", dirs_exist_ok=True)
        instructivo = f"# User Guide — {data.get('title', cliente)}\n\n## Credentials\n(to be completed)\n\n## Site Access\n- URL: {site_url or 'pending'}\n- Panel: (to be completed)\n\n## Next Steps\n1. Review site\n2. Changes\n3. Acceptance"
        (delivery_dir / "user_guide.md").write_text(instructivo, encoding="utf-8")
    ...
    def phase_seguimiento(self, cliente: str, dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 7: Follow-up — %s ===", cliente)
        data = self._get_cliente_data(cliente) or {"title": cliente}
        if dry_run: return {"ok": True, "dry_run": True}
        results = {}
        if TRELLO_API_KEY and TRELLO_TOKEN and TRELLO_API_KEY != "YOUR_TRELLO_API_KEY":
            try:
                ...
                board_id = next((b["id"] for b in boards if "marketing" in b.get("name", "").lower() or "projects" in b.get("name", "").lower()), boards[0]["id"] if boards else None)
                if board_id:
                    lists_resp = req.get(f"{base}/boards/{board_id}/lists", params=auth, timeout=15)
                    lists_resp.raise_for_status()
                    lists = lists_resp.json()
                    pending_id = next((l["id"] for l in lists if "pending" in l.get("name", "").lower() or "todo" in l.get("name", "").lower()), lists[0]["id"] if lists else None)
                    if pending_id:
                        card_data = {**auth, "idList": pending_id, "name": f"Follow-up — {data.get('title', cliente)}", "desc": f"Client: {data.get('title', cliente)}\nEmail: {data.get('primary_email', '')}\nTel: {data.get('phone', '')}"}
        ...
        try:
            ...
            if token_pickle.exists() and client_secret.exists():
                ...
                now = datetime.now()
                event = {
                    "summary": f"Meeting — {data.get('title', cliente)}",
                    "description": f"Follow-up with {data.get('title', cliente)}\nEmail: {data.get('primary_email', '')}",
    ...
    def phase_financiero(self, dry_run: bool = False) -> Dict:
        self.logger.info("=== PHASE 8: Financial Dashboard ===")
        if dry_run: return {"ok": True, "dry_run": True}
    ...
    def phase_status(self) -> Dict:
        self.logger.info("=== PHASE 9: Status ===")
    ...
    def main():
        ...
        if not args.cliente and not args.status and not args.financiero:
            parser.error("--cliente is required")
